...
- [x] **換場景後狀態列中文被切掉下半**(最嚴重的一件)。
  ...
  **量化重現**:走過 17 個畫面逐張量白底列數,換場景那一格從 16 掉到 10。
  修法:`GfxMenu` 記住最後一次的狀態列文字,`drawPicture()` 畫完後補畫回去
  (`chtRestoreStatus()`)。螢幕只有 200 列、遊戲佔 190 列,**15 列的列一定要吃掉圖的
  最上面 5 列**,沒有第三條路;補畫只是讓這個代價固定,不再看繪製順序。
  沒有狀態列的畫面(標題)不受影響——`kernelDrawMenuBar(true)` 會把快取清成空字串。
  驗收:26 張浸泡測試截圖(10 次換場景 + F1 + 道具欄 + 開關選單)白底列數全部 16。
